...
logo =\bcplume,
  noborder=true]
{\textrm{{\Large{PREVERJANJE 1.P.5} \Huge{}:
...
c.) $3^{2x}=5^{\,x+1}$ \quad (re\v{s}itev zapi\v{s}i v obliki logaritma $\log_a b$)
\end{naloga}\newpage

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\def\tockeB{2+1+1+3}
\begin{naloga}[\hfill $\tockeB$] \addtocounter{tocke}{\numexpr\tockeB\relax}
Podana je funkcija $f(x)=\left(\dfrac{1}{3}\right)^{x-2}-3$.
...
$$\log_a a^2+\log_a \sqrt[5]{a^3}-\log_3 2a\cdot\log_{2a} 81$$
\end{naloga}\newpage
...
Dolo\v{c}i konstante $a$, $b$ in $c$ ter izra\v{c}unaj $\dfrac{f(30)+1}{f(6)+1}$.
...
\draw[line width=1.pt,color=qqwuqq,smooth,samples=100,domain=-1.98:7.5] plot(\x,{ln((\x)+2)/ln(2)-1});
\draw [line width=1.pt,dash pattern=on 2pt off 2pt,color=ffqqqq] (-2,-3.5) -- (-2,3.5);
\begin{scriptsize}
\draw[color=qqwuqq] (6.7,2.9) node {$f$};
\end{scriptsize}
\end{axis}
\end{tikzpicture}
\end{center}
...
\begin{enumerate}
\item a.) $x=3$ \quad b.) $x=7$ \quad c.) $x=\log_{\frac{9}{5}} 5$
\item a.) ni\v{c}la $x=1$, za\v{c}etna vrednost $f(0)=6$, zaloga vrednosti $(-3,\infty)$ \quad b.) $f^{-1}(x)=\log_{\frac{1}{3}}(x+3)+2$, $D_{f^{-1}}=(-3,\infty)$
\item $-\dfrac{7}{5}$
\item $a=2$, $b=2$, $c=-1$; \quad $\dfrac{f(30)+1}{f(6)+1}=\dfrac{5}{3}$
\end{enumerate}
\end{document}
