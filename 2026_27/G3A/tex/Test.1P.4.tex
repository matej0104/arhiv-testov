...
logo =\bcplume,
  noborder=true]
{\textrm{{\Large{PREVERJANJE 1.P.4} \Huge{}:
...
a.) $\log_4(x+6)-\log_4 x=1$

b.) $9^{\,x-1}\cdot\sqrt{3}=27^{\frac{x}{3}}$

c.) $6^{x}=4^{\,x+3}$ \quad (re\v{s}itev zapi\v{s}i v obliki logaritma $\log_a b$)
\end{naloga}\newpage

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\def\tockeB{2+1+1+3}
\begin{naloga}[\hfill $\tockeB$] \addtocounter{tocke}{\numexpr\tockeB\relax}
Podana je funkcija $f(x)=2^{\,x+2}-8$.
...
$$\log_a a^6+\log_a \sqrt[3]{a}-\log_2 3a\cdot\log_{3a} 32$$
\end{naloga}\newpage

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\def\tockeD{4}
\begin{naloga}[\hfill $\tockeD$] \addtocounter{tocke}{\numexpr\tockeD\relax}
Na sliki je graf funkcije $f(x)=\log_b(x+a)+c$ z navpi\v{c}no asimptoto $x=-1$.
Dolo\v{c}i konstante $a$, $b$ in $c$ ter izra\v{c}unaj $\dfrac{f(8)-2}{f(2)-2}$.
...
xtick={-2.0,-1.0,...,9.0},
ytick={-1.0,0.0,...,5.0},]
\clip(-2.5,-1.5) rectangle (9.5,5.5);
\draw[line width=1.pt,color=qqwuqq,smooth,samples=100,domain=-0.98:9.5] plot(\x,{ln((\x)+1)/ln(3)+2});
\draw [line width=1.pt,dash pattern=on 2pt off 2pt,color=ffqqqq] (-1,-1.5) -- (-1,5.5);
\begin{scriptsize}
\draw[color=qqwuqq] (8.7,4.9) node {$f$};
\end{scriptsize}
\end{axis}
\end{tikzpicture}
\end{center}
...
\begin{enumerate}
\item a.) $x=2$ \quad b.) $x=\dfrac{3}{2}$ \quad c.) $x=\log_{\frac{3}{2}} 64$
\item a.) ni\v{c}la $x=1$, za\v{c}etna vrednost $f(0)=-4$, zaloga vrednosti $(-8,\infty)$ \quad b.) $f^{-1}(x)=\log_2(x+8)-2$, $D_{f^{-1}}=(-8,\infty)$
\item $\dfrac{4}{3}$
\item $a=1$, $b=3$, $c=2$; \quad $\dfrac{f(8)-2}{f(2)-2}=2$
\end{enumerate}
\end{document}
